...
class ShopDetailView extends StatefulWidget {
  const ShopDetailView({Key? key, required this.item}) : super(key: key);
  final Shop item;

  @override
  State<ShopDetailView> createState() => _ShopDetailView();
}

class _ShopDetailView extends State<ShopDetailView> {
  // var employees;
  // var services;
  // Employee? employees;
      ...
      body: SafeArea(child: SingleChildScrollView(
        ...
        return Column(
          children: [
                                                  ...
                                                  onClick: () async {
                                                    ...
                                                    if (respose) {
                                                      ToastMessages.showSuccess(
                                                          "Shop has been ${widget.item.status == 1 ? "blocked" : "unblocked"}");
            ...
            for (var i = 0; i < widget.item.products!.length; i++)
              shopProductItem(
                cardHeight,
                context,
                shop: widget.item,
                product: widget.item.products![i],
              )
          ],
        );
      }))),

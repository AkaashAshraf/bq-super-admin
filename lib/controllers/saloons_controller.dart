...
class SaloonsController extends GetxController {
  ...
  @override
  void onInit() {
    fetchSaloons(type: "all");
    fetchSaloons(type: "active");
    fetchSaloons(type: "deactive");

    super.onInit();
  }

  fetchSaloons({required String type}) async {
    try {
      loading(true);
  ...
  blockUnblockSaloon({required int status, required int saloonID}) async {
    try {
      loading(true);
